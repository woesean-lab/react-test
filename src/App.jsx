...
const fallbackTemplates = [
  { label: "Hoþ geldin", value: "Hoþ geldin! Burada herkese yer var.", category: "Karþýlama" },
  {
    label: "Bilgilendirme",
    value: "Son durum: Görev planlandýðý gibi ilerliyor.",
    category: "Bilgilendirme",
  },
  { label: "Hatýrlatma", value: "Unutma: Akþam 18:00 toplantýsýna hazýr ol.", category: "Hatýrlatma" },
]
...
const initialProblems = [
  { id: 1, username: "@ornek1", issue: "Ödeme ekranda takýldý, 2 kez kart denemiþ.", status: "open" },
  { id: 2, username: "@ornek2", issue: "Teslimat gecikmesi þikayeti.", status: "open" },
]
...
function App() {
    ...
    ;(async () => {
      try {
        ...
        if (!listLoadErrorRef.current) {
          ...
          toast.error("Liste verileri alýnamadý (API/DB kontrol edin).")
        }
      } finally {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        toast.error("Problem listesi alýnamadý (API/DB kontrol edin)")
      }
    })()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        toast.error("Stok listesi alýnamadý (API/DB kontrol edin)")
      }
    })()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        toast.error("Sunucuya baðlanýlamadý. (API/DB kontrol edin)")
      } finally {
  ...
  const handleTemplateChange = async (nextTemplate, options = {}) => {
    ...
    if (tpl && options.shouldCopy) {
      try {
        ...
        toast.success("Þablon kopyalandý", { duration: 1600, position: "top-right" })
        toast(
        ...
        toast.error("Kopyalanamadý", { duration: 1600, position: "top-right" })
      }
    }
  }
  ...
  const handleActiveTemplateEditSave = async () => {
    ...
    try {
      ...
      setIsEditingActiveTemplate(false)
      toast.success("Þablon güncellendi")
    } catch (error) {
      console.error(error)
      toast.error("Þablon güncellenemedi (API/DB kontrol edin).")
    } finally {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (res.status === 409) {
        toast("Var olan þablon aktif edildi", { position: "top-right" })
      ...
      toast.success("Yeni þablon eklendi")
    } catch (error) {
      console.error(error)
  ...
  const handleDeleteTemplate = async (targetLabel = selectedTemplate) => {
    if (templates.length <= 1) {
      toast.error("En az bir þablon kalmalý.")
      return
    }
    const target = templates.find((tpl) => tpl.label === targetLabel)
    const targetId = target?.id
    if (!targetId) {
      toast.error("Silinecek þablon bulunamadý.")
      return
    }
  ...
  const handleDeleteWithConfirm = (targetLabel) => {
    ...
    toast("Silmek için tekrar týkla", { position: "top-right" })
  }
  ...
  const handleListCreate = async () => {
    ...
    try {
      ...
      setListName("")
      toast.success("Liste oluþturuldu")
    } catch (error) {
      console.error(error)
      toast.error("Liste oluþturulamadý (API/DB kontrol edin).")
    }
  }
  ...
  const handleProductAdd = async () => {
    ...
    if (!name) {
      toast.error("Ürün ismi boþ olamaz.")
      return
    }
  ...
  const handleStockAdd = async () => {
    ...
    if (codes.length === 0) {
      toast.error("Anahtar kodu boþ olamaz.")
      return
    }
  ...
  const handleBulkCopyAndDelete = async (productId) => {
    ...
    try {
      ...
      toast.success(`${codes.length} stok kopyalandý ve silindi`, { duration: 1800, position: "top-right" })
    } catch (error) {
      console.error(error)
  ...
  const handleProductDeleteWithConfirm = async (productId) => {
    if (confirmProductTarget === productId) {
      try {
        ...
        toast.success("Ürün ve stoklarý silindi")
        return
      } catch (error) {
        console.error(error)
        toast.error("Silinemedi (API/DB kontrol edin).")
  ...
  const handleEditSave = async (productId) => {
    ...
    if (selectedTemplate && !templateValue) {
      toast.error("Geçerli teslimat mesajý bulunamadý.")
      return
    }
  ...
  const handleUndoDelete = async () => {
    ...
    try {
      ...
      setLastDeleted(null)
      toast.success("Silinen kayýt geri alýndý", { duration: 1400, position: "top-right" })
    } catch (error) {
      console.error(error)
      toast.error("Geri alýnamadý (API/DB kontrol edin).")
    }
  }
  const handleProductCopyMessage = async (productId) => {
    ...
    if (!message) {
      toast.error("Bu ürüne teslimat mesajý eklenmemiþ.")
      return
    }
    try {
      await navigator.clipboard.writeText(message)
      toast.success("Teslimat mesajý kopyalandý", { duration: 1500, position: "top-right" })
    } catch (error) {
      console.error(error)
      toast.error("Kopyalanamadý")
    }
  }
  ...
  const handleStockCopy = async (code) => {
    try {
      ...
      toast.success("Anahtar kopyalandý", { duration: 1500, position: "top-right" })
    } catch (error) {
      console.error(error)
      toast.error("Kopyalanamadý", { duration: 1500, position: "top-right" })
    }
  }
  ...
  const handleProblemAdd = async () => {
    ...
    if (!user || !issue) {
      toast.error("Kullanýcý adý ve sorun girin.")
      return
    }
    try {
  ...
  const handleProblemReopen = async (id) => {
    try {
      ...
      const updated = await res.json()
      setProblems((prev) => prev.map((p) => (p.id === id ? updated : p)))
      toast.success("Aktif probleme taþýndý")
    } catch (error) {
      console.error(error)
      toast.error("Güncellenemedi (API/DB kontrol edin).")
    }
  ...
  const handleProblemCopy = async (text) => {
    try {
      ...
      toast.success("Kullanýcý adý kopyalandý", { duration: 1400, position: "top-right" })
    } catch (error) {
      console.error(error)
      toast.error("Kopyalanamadý", { duration: 1600, position: "top-right" })
    }
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="max-w-2xl text-sm text-slate-200/80 md:text-base">
                      Kendi tonunu bul, hazýr þablonlarýný hýzla düzenle ve tek týkla ekibinle paylaþ.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2.5">
                    <span className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-accent-200 md:text-sm">
                      ...
                      Þablon: {templateCountText}
                    </span>
                    <span className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-accent-200 md:text-sm">
                      <span className="h-2 w-2 rounded-full bg-amber-300" />
                      Kategori sayýsý: {categoryCountText}
                    </span>
                    <span className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-accent-200 md:text-sm">
                      <span className="h-2 w-2 rounded-full bg-amber-300" />
                  ...
                  <div className="relative rounded-2xl border border-white/10 bg-white/10 p-6 shadow-glow backdrop-blur-md">
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="font-display text-2xl font-semibold text-white">
                            {activeTemplate?.label || (showLoading ? "Yükleniyor..." : "Yeni þablon")}
                          </h3>
                          <span
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        >
                          {isEditingActiveTemplate ? "Vazgeç" : "Mesajý düzenle"}
                        </button>
                        <button
                          type="button"
                    ...
                    {isEditingActiveTemplate ? (
                      <textarea
                        ...
                        placeholder="Mesaj içeriðini güncelle"
                      ...
                      <p className="mt-3 text-sm leading-relaxed text-slate-200/90">
                        {activeTemplate?.value ||
                          (showLoading ? "Veriler yükleniyor..." : "Mesajýný düzenleyip kaydetmeye baþla.")}
                      </p>
                    )}
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="space-y-6 lg:col-span-2">
                <div className={`${panelClass} bg-ink-800/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-300/80">Þablon listesi</p>
                      <p className="text-sm text-slate-400">Baþlýklarýna dokunarak düzenle ve kopyala.</p>
                    </div>
                          ...
                          <div key={cat} className="rounded-2xl border border-white/10 bg-ink-900/60 p-3 shadow-inner">
                            ...
                            >
                              <span className="inline-flex items-center gap-2">
                                ...
                                <span className="text-xs text-slate-400">{list.length} þablon</span>
                              </span>
                              <span
              ...
              <div className="space-y-6">
                <div className={`${panelClass} bg-ink-800/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Yeni kategori ekle, ardýndan mesaj alanýndan seç.</p>
                    </div>
                ...
                <div className={`${panelClass} bg-ink-900/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-300/80">Þablon ekle</p>
                      <p className="text-sm text-slate-400">Baþlýk, kategori ve mesajý ekleyip kaydet.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">Hýzlý ekle</span>
                  </div>
                  ...
                  <div className="mt-4 space-y-4 rounded-xl border border-white/10 bg-ink-900/70 p-4 shadow-inner">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        placeholder="Örn: Karþýlama notu"
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-xs font-semibold text-slate-200">
                        ...
                        <span className="text-[11px] text-slate-400">Anlýk karakter: {messageLength}</span>
                      </div>
                      <textarea
                        id="message-mini"
                        value={message}
                ...
                <div className={`${panelClass} bg-ink-800/60`}>
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-300/80">Hýzlý ipuçlarý</p>
                  <ul className="mt-3 space-y-2 text-sm text-slate-300">
                    <li>- Baþlýk boþ kalýrsa otomatik bir isimle kaydedilir.</li>
                    <li>- Þablona týklamak metni panoya kopyalar.</li>
                    <li>- Kategori silince þablonlar “Genel”e taþýnýr.</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="space-y-2">
                  ...
                  <p className="max-w-2xl text-sm text-slate-200/80">
                    Yeni liste oluþtur, listeleri görüntüle ve hücreleri Excel benzeri biçimde düzenle.
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="space-y-6 lg:col-span-2">
                <div className={`${panelClass} bg-ink-800/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Listeye týkla ve tabloyu aç.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                      {listCountText} liste
                    </span>
                ...
                <div className={`${panelClass} bg-ink-900/60`}>
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-300/80">Liste içeriði</p>
                      <p className="text-sm text-slate-400">Hücreleri seçip düzenleyebilirsin.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
                        <span>Baþlýklara sað týkla: ekle/sil</span>
                        {listSavedAt ? (
                  ...
                  {!activeList ? (
                    <div className="mt-4 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-400">
                      Bir liste seçin veya yeni liste oluþturun.
                    </div>
                  ) : (
                    <>
              ...
              <div className="space-y-6">
                <div className={`${panelClass} bg-ink-900/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Liste adýný girip oluþtur.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                      {listCountText} liste
                    </span>
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        placeholder="Örn: Haftalýk rapor"
                        className="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-accent-400 focus:outline-none focus:ring-2 focus:ring-accent-500/30"
                      />
                    </div>
                    <button
                ...
                <div className={`${panelClass} bg-ink-900/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Aktif listenin adýný deðiþtir ya da sil.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                ...
                <div className={`${panelClass} bg-ink-800/60`}>
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-300/80">Ýpuçlarý</p>
                  <ul className="mt-3 space-y-2 text-sm text-slate-300">
                    <li>- Yeni liste varsayýlan bir tabloyla baþlar.</li>
                    <li>- Satýr/sütun ekleyerek tabloyu geniþlet.</li>
                    <li>- Bir hucreye cok satir yapistirinca asagiya yayilir.</li>
                    <li>- Formül için "=" ile baþla (örn: =SUM(A1:A5)).</li>
                    <li>- Desteklenenler: SUM, AVERAGE, MIN, MAX, COUNT.</li>
                    <li>- Satýr/sütun baþlýðýna sað týkla: ekle/sil.</li>
                    <li>- Satir/sutun secmek icin basliga tikla; Shift aralik, Ctrl tek tek.</li>
                    <li>- Veriler veritabanýnda saklanýr.</li>
                  </ul>
                </div>
              </div>
            </div>
                ...
                <div className="min-w-[180px] rounded-xl border border-white/10 bg-ink-900/95 p-2 text-xs text-slate-100 shadow-card backdrop-blur">
                  {listContextMenu.type === "row" && (
                    <>
                      ...
                      >
                        Satýr ekle
                        <span className="text-[10px] text-slate-400">Altýna</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                  ...
                  {listContextMenu.type === "column" && (
                    <>
                      ...
                      >
                        Sütun ekle
                        <span className="text-[10px] text-slate-400">Saðýna</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => {
            ...
            <header className="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-ink-900 via-ink-800 to-ink-700 p-6 shadow-card">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="space-y-2">
                  ...
                  <p className="max-w-2xl text-sm text-slate-200/80">
                    Anahtarlarý görsel olarak tut, kopyala, ekle ve sil. Bu bölüm veri tabanýna baðlý çalýþýr.
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-accent-200">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-ink-900/60 p-4 shadow-card">
                ...
                <div className="relative">
                  ...
                  <p className="mt-1 text-xs text-slate-400">Kayýtlý ürün sayýsý</p>
                </div>
              </div>
              <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-ink-900/60 p-4 shadow-card">
                ...
                <div className={`${panelClass} bg-ink-800/60`}>
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Stoklarý satýr bazýnda yönet, toplu iþlem yap.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              aria-label="Ürün detaylarýný aç/kapat"
                            >
                              &gt;
                            </button>
                          </div>
                        ...
                        {openProducts[product.id] && (
                          <div className="mt-4 space-y-3">
                            <div className="flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-ink-900/60 px-3 py-2 text-xs text-slate-300">
                              ...
                              >
                                {confirmProductTarget === product.id ? "Silmek için tekrar týkla" : "Ürünü sil"}
                              </button>
                            </div>
                            {editingProduct[product.id] && (
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-slate-400">Saðdan ürün yarat, solda stoklarý görün.</p>
                      </div>
                      <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                    ...
                    <div className="mt-4 space-y-4 rounded-xl border border-white/10 bg-ink-900/70 p-4 shadow-inner">
                      ...
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          placeholder="Her satýr bir anahtar / kod, örn: XXXX-XXXX-XXXX-XXXX"
                          className="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-accent-400 focus:outline-none focus:ring-2 focus:ring-accent-500/30"
                        />
                      </div>
            ...
            <header className="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-ink-900 via-ink-800 to-ink-700 p-6 shadow-card">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="space-y-2">
                  ...
                  <h1 className="font-display text-3xl font-semibold text-white">Problemli Müþteriler</h1>
                  <p className="max-w-2xl text-sm text-slate-200/80">
                    Müþteri kullanýcý adý ve sorununu kaydet; çözülünce “Problem çözüldü” ile kapat veya sil.
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-accent-200">
                    Açýk problem: {openProblems.length}
                  </span>
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-accent-200">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="space-y-6 lg:col-span-2">
                <div className={`${panelClass} bg-ink-800/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-300/80">Açýk problemler</p>
                      <p className="text-sm text-slate-400">Kullanýcý adý ve sorun bilgisi listelenir.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                      {openProblems.length} kayýt
                    </span>
                  </div>
                ...
                <div className={`${panelClass} bg-ink-900/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Çözülmüþ kayýtlarý sakla ya da sil.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                      {resolvedProblems.length} kayýt
                    </span>
                  </div>
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className={`${panelClass} bg-ink-900/60`}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Kullanýcý adý ve sorunu yazýp kaydet.</p>
                    </div>
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">
                      Toplam: {problems.length}
                    </span>
                  ...
                  <div className="mt-4 space-y-4 rounded-xl border border-white/10 bg-ink-900/70 p-4 shadow-inner">
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        rows={4}
                        placeholder="Sorunun kýsa özeti..."
                        className="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-accent-400 focus:outline-none focus:ring-2 focus:ring-accent-500/30"
                      />
                    </div>
